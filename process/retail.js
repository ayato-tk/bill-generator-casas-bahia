...
async function init(browser, url) {
  try {
    ...
    await browser.close();
    const newBrowser = await puppeteer.launch({ headless: false, timeout: 0 });
    return init(newBrowser, url);
  }
}

export async function downloadRetailFile() {
  ...
  if (recaptcha) {
    const solution = await ac.solveRecaptchaV2Proxyless(
      process.env.URL,
